...
return new class extends Migration
{
    ...
    public function up()
    {
        $trigger = "CREATE TRIGGER `after_update` AFTER UPDATE ON `renovations` FOR EACH ROW BEGIN

                        Declare Req
                        	Cursor for

                        		SELECT new.id, new.label, new.year, new.MONTH, new.estimated_quantity,
                        		@KWH_debut := if( new.MONTH < 12, ROUND(((new.estimated_quantity * 4.694 * (DATEDIFF( CONCAT( new.YEAR,'-',12,'-',31 ),CONCAT(new.YEAR, '-', new.MONTH + 1,'-',01)))* 5.17 * 24) / 1000),2) , 'null') AS KWH_debut,
                        		@KWH_apres_travaux := if( new.MONTH < 12, ROUND(((new.estimated_quantity * 0.22 * (DATEDIFF( CONCAT( new.YEAR,'-',12,'-',31 ),CONCAT(new.YEAR, '-', new.MONTH + 1,'-',01))) * 5.17 * 24) / 1000),2) , 'null') AS KWH_apres_travaux,
                        ...
                        Declare Req2
                        	Cursor for
                        		...
                        		@KWH_debut := ROUND(((new.estimated_quantity * 4.694 * (DATEDIFF( CONCAT( new.YEAR,'-',12,'-',31 ),CONCAT(new.YEAR, '-', 01,'-',01))) * 5.17 * 24) / 1000),2) AS KWH_debut,
                        		@KWH_apres_travaux := ROUND(((new.estimated_quantity * 0.22 * (DATEDIFF( CONCAT( new.YEAR,'-',12,'-',31 ),CONCAT(new.YEAR, '-', 01,'-',01))) * 5.17 * 24) / 1000),2) AS KWH_apres_travaux,
                        		@CO_Debut := ROUND(@KWH_debut * 0.232, 2) as CO_Debut,
                        		@CO_apres_travaux := ROUND(@KWH_apres_travaux * 0.232, 2) AS CO_apres_travaux
                        		FROM renovations
                        		WHERE renovations.year = new.year and new.label LIKE '%toiture%' AND new.purpose LIKE 'Economies' AND new.estimated_quantity > 0 AND new.MONTH <> 12 and renovations.id = new.id;
                        		...
                        		open nb_records;
                        			if (@nb_records > 0 )
                        			then
                        				update calculo2s set kwh_before = round (@KWH_debut, 2), kwh_after = round (@KWH_apres_travaux, 2),
